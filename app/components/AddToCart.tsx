...
const AddToCart = ({
  ...
  const [itemFinish, setItemFinish] = useState<string>(
  ...
  const bitEnds: string[] = [
    "Leave as is",
    "w/ Slobber Bar",
    `${purchaseOption === standAloneMoutpieceOptions[1] ? "w/ Replacement Slobber Bar (+$60)" : ""} `,
    "w/ Rein Hooks & Chains",
    "w/ O-Rings",
    "w/ Stirrups & Chains",
  ];
  ...
  const spadeMenuOptions = {
    ...
    positionOfBraces: ["Position A (Center of Cheek)", "Position B"],
    mpHeight: [`3"`, `3-1/4"`, `3-1/2"`, `3-3/4"`, `4"`],
  };

  const range = new Array(66).fill(null);

  const spadeExclusionList: string[] = [
    "B-1053",
    "B-1569",
    "B-1587",
    "B-1588",
  ...
  const upchargeList: (number | string)[] = [
    13,
    14,
    17,
    18,
      ...
      <dialog id="my_modal_1" className="modal">
        <div className="modal-box space-y-2">
            ...
            <>
              <span>
                ...
                <span className="flex items-center gap-4">
                  <select
                    onChange={(e) => {
                      setPurchaseOption(e.currentTarget.value);
                    }}
          ...
          {(currentlySelectedItem?.name.includes("B-") &&
            !currentlySelectedItem?.name.includes("25R") &&
            !currentlySelectedItem?.name.includes("26R") &&
              ...
              <span>
                ...
                <span className="flex items-center gap-4">
                  <select
                    ...
                    value={itemStyle}
                    onChange={(e) => {
                      setItemStyle(e.currentTarget.value);
                    }}
                    className="select mb-6"
                  >
                    {!currentlySelectedItem?.categories?.includes("Snaffle")
                      ? [...range, "MP-25R (+$50)", "MP-26R (+$50)"].map(
                          (i, key) => {
                            return (
                              <option
                                key={key}
                                value={
                                  typeof i !== "string"
                                    ? `MP-${key + 1 < 10 ? "0" : ""}${(key + 1).toString()}${upchargeList.includes(key + 1) ? `${key + 1 === 66 ? " (+$60)" : " (+$35)"}` : ""}`
                                    : `${i}`
                                }
                              >
                                {typeof i !== "string"
          ...
          {currentlySelectedItem?.categories?.includes("Mouthpiece") ||
          currentlySelectedItem?.categories?.includes("Horse Bit") ||
          currentlySelectedItem?.categories?.includes("Snaffle") ? (
            <span>
              <p className="pb-1">Select width</p>
              <select
                defaultValue="Select width"
                value={itemWidth}
                onChange={(e) => {
                  setItemWidth(e.currentTarget.value);
                }}
                className="select mb-6"
              >
          ...
          {purchaseOption === standAloneMoutpieceOptions[1] ||
          currentlySelectedItem?.categories?.includes("Snaffle") ||
          currentlySelectedItem?.categories?.includes("Horse Bit") ? (
            <>
              {purchaseOption === standAloneMoutpieceOptions[1] ||
                ...
                <>
                  <span>
                    ...
                    <select
                      value={itemMovement}
                      onChange={(e) => {
                        setItemMovement(e.currentTarget.value);
                      }}
                      className="select mb-6"
                    >
                  ...
                  <span>
                    ...
                    <span className="flex items-center gap-4">
                      <select
                        value={itemAngle}
                        onChange={(e) => {
                          setItemAngle(e.currentTarget.value);
                        }}
                        className="select mb-6"
                      >
                  ...
                  <span>
                    ...
                    <span className="flex items-center gap-4">
                      <select
                        value={bitEnds}
                        onChange={(e) => {
                          setItemBitEndsWIth(e.currentTarget.value);
                        }}
                        className="select mb-6"
                      >
          ...
          itemStyle.includes("$") ? (
            <>
              <span>
                ...
                <span className="flex items-center gap-4">
                  <select
                    value={itemCopperOnSpoon}
                    onChange={(e) => {
                      setItemCopperOnSpoon(e.currentTarget.value);
                    }}
                    className="select mb-6"
                  >
              ...
              {purchaseOption !== standAloneMoutpieceOptions[0] ? (
                <span>
                  <p className="pb-1">Position of braces</p>
                  <span className="flex items-center gap-4">
                    <select
                      value={itemPositionOfBraces}
                      onChange={(e) => {
                        setItemPositionOfBraces(e.currentTarget.value);
                      }}
                      className="select mb-6"
                    >
              ...
              <span>
                ...
                <select
                  value={itemHeight}
                  onChange={(e) => {
                    setItemHeight(e.currentTarget.value);
                  }}
                  className="select mb-6"
                >
            ...
            <>
              <span>
                ...
                <select
                  value={itemFinish}
                  onChange={(e) => {
                    setItemFinish(e.currentTarget.value);
                  }}
                  className="select mb-6"
                >
